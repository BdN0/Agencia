...
namespace POO
{
    public class Cidade
    {
        ...
        {
            dao.ExecutarQuery("delete from cidade where codigo = " + codigo);
        }
        ...
        {
            return dao.ExecutarConsulta("Select * from cidade where uf = "+ uf);
        }
        public DataTable Visualizar_Cidade()
        {
            return dao.ExecutarConsulta("Select * from cidade where codigo= " + codigo);
        }
        public DataTable Pesquisar_Cidade()
        {
            if (nome != null && uf!=null)
            {
                return dao.ExecutarConsulta("Select * from cidade where nome =" + nome + " and uf = " + uf);
            }
            else if (nome != null)
            {
                return dao.ExecutarConsulta("Select *  from  cidade = " + nome);
            }
            else
            {
                return dao.ExecutarConsulta("Select *  from  cidade = " + uf);
            }
        }
